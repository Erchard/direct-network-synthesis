...
| DNS05-BM | Can a streaming Nystrom-style RBF map preserve compact quality with lower peak memory? | Existing failure-scaling train/validation boundaries only; no test evaluation | dns05-bounded-memory-kernel-map-protocol.md | Protocol drafted, not evaluated |
